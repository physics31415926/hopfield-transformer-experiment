"""
Ablation Study: Effect of Hopfield Iteration Steps (T)

Tests T=1,2,3,5,8 on all three tasks to understand how
the number of Hopfield update steps affects performance.
...
from run_experiments import (
    AssociativeRecallDataset, NoisyCopyDataset, CharLMDataset,
...
def run_ablation(args):
    ...
    if args.experiment in ('all', 'copy'):
        experiments['copy'] = {
            'task': 'copy',
            'cls': NoisyCopyDataset,
            'kwargs': dict(num_samples=5000, seq_len=32, vocab_size=64, mask_ratio=0.15),
        }
    ...
    for exp_name, exp_cfg in experiments.items():
        ...
        for mode in modes_to_test:
            for T in steps_list:
                ...
                model = build_model(
                    mode=mode, hopfield_steps=T, **common_model_kwargs
                ).to(device)
    ...
    for exp_name, exp_results in all_results.items():
        print(f"\n  {exp_name}:")
        print(f"  {'Config':<20} {'Params':>10} {'Best Val Loss':>14} {'Time':>8}")
        print(f"  {'-'*55}")
        for key, r in sorted(exp_results.items()):
            print(f"  {key:<20} {r['params']:>10,} {r['best_val_loss']:>14.4f} {r['time']:>7.1f}s")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Hopfield Steps Ablation Study')
